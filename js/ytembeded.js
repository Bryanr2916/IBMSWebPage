    ...
    init: function(cfg) {
        ...
        if (!this.cfg.block) {
            ...
            if (!this.cfg.type) {
                ...
                //this.message('Cargando videos de YouTube. Por favor espere...');
        ...
        if (json.error) {
            ...
            for (var i = 0; i < json.items.length; i++) {
                ...
                if (this.cfg.layout == 'thumbnails') {
                    ...
                    //quite la descripcion
                    //this.cfg.layout = full
                    li.innerHTML = '<table cellspacing="0" cellpadding="0" border="0"><tr><td valign="top" rowspan="2"></td><td valign="top"><h3>' + entry.snippet.title + '</h3><div>' + (entry.contentDetails ? 'Duración: ' + ytEmbed.formatDuration(entry.contentDetails.duration) + '<br>' : '')+ 'De: <a target="blank_" href="https://www.youtube.com/channel/' + entry.snippet.channelId + '">' + entry.snippet.channelTitle + '</a></div></td></tr>';
